...
Rs.Close
Rs.Open "SELECT ISNULL(MAX(Value), 0) FROM Tags_History WHERE (TagID='Main1') AND (DT > GETDATE()-1.0/12)", Conn '
...
Rs.Open "SELECT ISNULL(Max(Value), 0) FROM Tags_History WHERE (TagID='Main2All24') AND (DT > GETDATE()-1.0/12)", Conn 'Недоступность всех АТМ
Value2A_max=Rs.Fields(0)
Rs.Close

Rs.Open "SELECT ISNULL(Max(Value), 0) FROM Tags_History WHERE (TagID='Main2All') AND (DT > GETDATE()-1.0/12)", Conn 'Недоступность всех АТМ
if Value2A_max<Rs.Fields(0) then
Value2A_max=Rs.Fields(0)
end if
Rs.Close

Rs.Open "SELECT ISNULL(Max(Value), 0) FROM Tags_History WHERE (TagID='Main2AllBPT24') AND (DT > GETDATE()-1.0/12)", Conn 'Недоступность всех БПТ
Value2AllBPT_max=Rs.Fields(0)
Rs.Close

Rs.Open "SELECT ISNULL(Max(Value), 0) FROM Tags_History WHERE (TagID='Main2AllBPT') AND (DT > GETDATE()-1.0/12)", Conn 'Недоступность всех БПТ
  ...
  for j=1 to 2
	...
	if Colors2(i, j)=0 then 
	  ...
	  if Colors2(i, j)=1 then 
        ...
        Colors2(i, j)="color: #000000; background-color: "&clWarning&";"
	  else
	    'Colors2(i, j)=clError
        Colors2(i, j)="color: #ffffff; background-color: "&clError&";"
	  end if
	end if
  next
next
...
"WHERE (A.DT > GETDATE()-1.0/12) AND "&_
"	(A.CHANNEL_ID in (SELECT DISTINCT CHANNEL_ID FROM vw_Channel_History WHERE [Value]=0 AND DT > GETDATE()-1.0/12)) "&_
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart1 = new Highcharts.Chart({
					...
					title:   {align: 'right', text: '<a style="text-decoration: underline;" href="detail.asp?T=0" >Неуспешные операции</a>'},
					...
					{
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main1') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  Response.Write("data: [")
...
<%
LastTime1=Now
Rs.Open "SELECT top 1 DT FROM Tags_History WHERE (TagID='Main1all') AND (DT > GETDATE()-1.0/12) ORDER BY DT desc", Conn
if not Rs.Eof then
	LastTime1=Rs.Fields("DT")
end if
Rs.Close

Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main1all') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
'LastTime1=Now
if not Rs.Eof then
  Response.Write(", data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Replace(FormatNumber(Rs.Fields("Value"), 1, -1, 0, 0), ",", ".")&"]")
...
<%
LastTime2=Now
Rs.Open "SELECT top 1 DT FROM Tags_History WHERE (TagID='Main2All') AND (DT > GETDATE()-1.0/12) ORDER BY DT desc", Conn
if not Rs.Eof then
	LastTime2=Rs.Fields("DT")
end if
Rs.Close

Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2All') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
'LastTime2=Now
if not Rs.Eof then
  Response.Write(", data: [")
...
%>
						...
						yAxis: 1
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2Fil') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  Response.Write(", data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Rs.Fields("Value")&"]")
  Rs.MoveNext
...
%>
						...
						yAxis: 1
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2Centr') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  Response.Write(", data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Rs.Fields("Value")&"]")
  Rs.MoveNext
...
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2All24') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  Response.Write(" data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Rs.Fields("Value")&"]")
  Rs.MoveNext
...
<%
LastTime4=Now
Rs.Open "SELECT top 1 DT FROM Tags_History WHERE (TagID='Main2AllBPT') AND (DT > GETDATE()-1.0/12) ORDER BY DT desc", Conn
if not Rs.Eof then
	LastTime4=Rs.Fields("DT")
end if
Rs.Close

Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2AllBPT') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
'LastTime4=Now
if not Rs.Eof then
  Response.Write(", data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Rs.Fields("Value")&"]")
...
%>
						...
						yAxis: 1
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2FilBPT') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  Response.Write(", data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Rs.Fields("Value")&"]")
  Rs.MoveNext
...
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main2AllBPT24') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  Response.Write(" data: [")
  Response.Write("[Date.UTC("&DateTimeFormat(Rs.Fields("DT"), "yyyy, mm, dd, hh, nn")&"), "&Rs.Fields("Value")&"]")
  Rs.MoveNext
...
%>
				...
				chart3 = new Highcharts.Chart({
					...
					legend:  {enabled: false},
					tooltip: {enabled: false},
					title:   {align: 'right', text: '<a style="text-decoration: underline;" href="detail.asp?T=1" >Упавшие каналы:</a> <span style="color: #FF0000"><% =Value3Down %> </span>'},
...
<%
LastTime3=Now
Rs.Open "SELECT top 1 DT FROM Tags_History WHERE (TagID='Main3') AND (DT > GETDATE()-1.0/12) ORDER BY DT desc", Conn
if not Rs.Eof then
	LastTime3=Rs.Fields("DT")
end if
Rs.Close

Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, -1*[Value] as [Value] FROM Tags_History WHERE (TagID='Main3') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
'LastTime3=Now
if not Rs.Eof then
  m="marker: {fillColor: '#00FF00', lineColor: '#00FF00', radius: 3}, "
...
%>
					...
					},
					{
						...
						data: [
<%
Rs.Open "SELECT dateadd(month,-1,DT) DT, TagID, [Value] FROM Tags_History WHERE (TagID='Main3down') AND (DT > GETDATE()-1.0/12) ORDER BY DT", Conn
if not Rs.Eof then
  v=Rs.Fields("Value")
		...
		<td style="border: none;">
		  <div id="containerBB" style="width: 500px; height: 300px; margin: 0 auto; font-size: 20pt;">
			<table border="0" height="90%" width="100%" cellspacing="0">
			  <tr><td>&nbsp;</td>
			      <td style="border: solid 1px #C0C0C0; font-size: 24pt">&nbsp;&nbsp;&nbsp;<a style="color: #ffffff;" href="detail.asp?T=2" >АТМ</a>&nbsp;&nbsp;&nbsp;</td>
				  <td style="border: solid 1px #C0C0C0; font-size: 24pt">&nbsp;&nbsp;&nbsp;<a style="color: #ffffff;" href="detail.asp?T=4" >БПТ</a>&nbsp;&nbsp;&nbsp;</td>
			  </tr>
			  <tr>
			      <td class="Head" style="border: solid 1px #C0C0C0; color: #d7b6b6;" nowrap>LINK 24, %</td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(4,1) %>"><% =Text2(4,1) %></td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(4,2) %>"><% =Text2(4,2) %></td>
			  </tr>
			  <tr>
			      <td class="Head" style="border: solid 1px #C0C0C0; color: #66FFFF;" nowrap>ПЦ, %</td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(1,1) %>"><% =Text2(1,1) %></td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(1,2) %>"><% =Text2(1,2) %></td>
			  </tr>
			  <tr>
				  <td class="Head" style="border: solid 1px #C0C0C0; color: #FF66FF;" nowrap>&nbsp;ФИ, шт.&nbsp;</td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(2,1) %>"><% =Text2(2,1) %></td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(2,2) %>"><% =Text2(2,2) %></td>
			  </tr>
			  <tr>
				  <td class="Head" style="border: solid 1px #C0C0C0; color: #FFFF66;" nowrap><a style="color: #FFFF66;" href="detail.asp?T=3" >ЦСП, шт</a></td>
			      <td class="Txt" style="border: solid 1px #C0C0C0; <% =Colors2(3,1) %>"><% =Text2(3,1) %></td>
